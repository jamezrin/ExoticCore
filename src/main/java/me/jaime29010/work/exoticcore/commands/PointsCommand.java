        ...
        if (sender instanceof Player) {
            ...
            if (args.length == 1) {
                ...
                if (target != null) {
                    ...
                    player.sendMessage(Messager.colorize(main.getConfig().getString("messages.player-points-other")
                            .replace("%player%", name)
                            .replace("%points%", String.valueOf(wrapper.getPoints()))
                    ));
                } else {
                ...
                player.sendMessage(Messager.colorize(main.getConfig().getString("messages.player-points")
                        .replace("%points%", String.valueOf(wrapper.getPoints()))
                ));
                return true;
            }
            player.sendMessage(ChatColor.RED + "Usage: /points [player]");
        } else {
